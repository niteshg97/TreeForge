...
from src.tree.classifier import (
    DecisionTreeClassifier as TreeForgeClassifier,
)
...
def _benchmark_sklearn(
    ...
    clf = SklearnDecisionTreeClassifier(
        max_depth=max_depth,
        random_state=42,
    )
...
def run_benchmark(
    ...
    """Run TreeForge vs sklearn benchmark."""
    ...
    sklearn_result = _benchmark_sklearn(
        X_train,
        X_test,
        y_train,
        y_test,
...
def format_comparison_table(
    treeforge_result: BenchmarkResult,
    ...
    rows = [
        ...
        (
            "Accuracy",
            f"{treeforge_result.accuracy:.4f}",
            f"{sklearn_result.accuracy:.4f}",
        ),
        (
            "Precision",
            f"{treeforge_result.precision:.4f}",
            f"{sklearn_result.precision:.4f}",
        ),
        (
            "Recall",
            f"{treeforge_result.recall:.4f}",
            f"{sklearn_result.recall:.4f}",
        ),
        (
            "F1 Score",
            f"{treeforge_result.f1:.4f}",
            f"{sklearn_result.f1:.4f}",
        ),
        (
            "Fit Time (s)",
        ...
        (
            "Tree Depth",
            str(treeforge_result.depth),
            str(sklearn_result.depth),
        ),
        (
            "Leaves",
            str(treeforge_result.n_leaves),
            str(sklearn_result.n_leaves),
        ),
    ]
...
if __name__ == "__main__":
    treeforge_result, sklearn_result = run_benchmark()
